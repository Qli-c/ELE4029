...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case WhileStmt:
		{
          ...
          {
			InvalidConditionError(t->child[0]->lineno);
		  }
		...
		case ReturnStmt:
		{
			...
			************************************************************/
			...
			else if(t->child[0]->type != currentScope->func->type)
			{
				InvalidReturnError(t->lineno);
			}
			// Break
		...
		case CallExpr:
		{
			...
			************************************************************/
			...
			{
				if(paramNode->type == Void && argNode != NULL)
				{
					InvalidFunctionCallError(t->name, t->lineno);
				}

				else if(paramNode->type != argNode->type)
				{
					InvalidFunctionCallError(t->name, t->lineno);
				}
